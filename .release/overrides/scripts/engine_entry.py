"""Portable checkpoint metadata with byte-authenticated upstream prior provenance.

Network, architecture, vocabulary and metadata enter the semantic digest.
...
def install_prior_metadata_bridge():
    import importlib
    import logging
    from pathlib import Path
    adapter=importlib.import_module('reinvent.runmodes.create_adapter')
    upstream=adapter.check_metadata
    def verify_prior_or_checkpoint(filename,data):
        meta=data.get('metadata')
        current=meta.as_dict() if hasattr(meta,'as_dict') else (meta or {})
        if current.get('hash_id_format')!=FORMAT:
            path=Path(filename)
            if path.is_file() and hashlib.sha256(path.read_bytes()).hexdigest()==PRIOR_SHA256:
                data['metadata']=update(data,comment='Official prior bytes verified SHA256 '+PRIOR_SHA256,write_update=False)['metadata']
                logging.getLogger(__name__).info('Verified official prior bytes; portable metadata digest applied in memory')
        return upstream(filename,data)
    adapter.check_metadata=verify_prior_or_checkpoint
...
if __name__=='__main__':
    if len(sys.argv)>1 and sys.argv[1]=='--self-test':
        ...
        assert hashlib.sha256(path.read_bytes()).hexdigest()==PRIOR_SHA256, 'Unverified prior bytes'
        ...
        with tempfile.TemporaryDirectory() as directory:
            checkpoint=Path(directory)/'state.chkpt'; torch.save(sealed,checkpoint)
